            ...
            <div class="col-md-12 mx-auto">
              <div class="cta-inner rounded">
                <h2 class="section-heading  text-center mb-4">
                  <span class="section-heading-lower">上传好物分享</span>
                </h2>
                <div class="row">
                    <div class="col-md-12 mx-auto">
                    <div class="form-group  has-error">
                      <input class="form-control" id="upload_goods_input" type="file" placeholder="选择文件" accept="image/*">
                      <p class="help-block text-danger"></p>
                    </div>
                    <form id="upload_goods_form" name="upload_goods_form" novalidate="novalidate" action="{{route('goods.store')}}" method="POST">
                      <div class="row">
                        <div class="col-md-12">
                          {{ csrf_field() }}
                          ...
                          <div class="form-group {{ $errors->has('url') ? ' has-error' : '' }}">
                            <input class="form-control" type="text" name="url" placeholder="好物源地址" required="required" data-validation-required-message="您必须输入一个标题。">
                            @if ($errors->has('url'))
                                <span class="help-block">
...
<script>
  ...
  var subObject = { 
        next: function(response){
          ...
          $("#upload_goods_input").next().html('正在上传，上传进度：' + Math.floor(total.percent) / 100 + "%");
        },
        error: function(res){
          $("#upload_goods_input").next().html("上传出错，刷新重试！");
        },
        complete: function(res){
          // 上传成功！
          $("#upload_goods_input").next().html("上传成功！");
          $("#upload_btn").removeAttr("disabled");
        }
      };
  $("#upload_goods_input").change(function(){
    var file = this.files[0];
    var fileExt = (/[.]/.exec(file.name)) ? /[^.]+$/.exec(file.name.toLowerCase()) : '';
    var key =  (new Date()).valueOf() + "." + fileExt;
    $("#upload_goods_form input[name=size]").val(file.size / 1024 / 1024);
    $("#upload_goods_form input[name=image_url]").val(domain + "/" + key);
