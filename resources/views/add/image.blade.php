            ...
            <div class="col-md-12 mx-auto">
              <div class="cta-inner rounded">
                <h2 class="section-heading  text-center mb-4">
                  <span class="section-heading-lower">上传图片</span>
                </h2>
                <div class="row">
                    <div class="col-md-12 mx-auto">
                    <div class="form-group  has-error">
                      <input class="form-control" id="upload_image_input" type="file" placeholder="选择文件" accept="image/*">
                      <p class="help-block text-danger"></p>
                    </div>
                    <form id="upload_image_form" name="upload_image_form" novalidate="novalidate" action="{{route('image.store')}}" method="POST">
                      <div class="row">
                        <div class="col-md-12">
                          {{ csrf_field() }}
...
<script>
  ...
  var subObject = { 
        next: function(response){
          ...
          $("#upload_image_input").next().html('正在上传，上传进度：' +  Math.floor(total.percent) / 100  + "%。");
        },
        error: function(res){
          $("#upload_image_input").next().html("上传出错，刷新重试！");
        },
        complete: function(res){
          // 上传成功！
          $("#upload_image_input").next().html("上传成功！");
  ...
  $("#upload_image_input").change(function(){
    ...
    $("#upload_image_form input[name=size]").val(file.size / 1024 / 1024);
    $("#upload_image_form input[name=url]").val(domain + "/" + key);
